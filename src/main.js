...
const input = new Input($('game-canvas'), {
  ...
  action: action => { if(action === 'pause') pauseGame(); else if(action === 'bomb') game?.useBomb(); else if(action === 'weapon') game?.switchWeapon(); else if(action === 'drive') game?.activateDrive(); },
...
function updateBest() {
  const best = save.records[`${save.difficulty}:${save.stage}`] || 0;
...
function updateHud() {
  ...
  setText('hud-sector',`SECTOR 0${game.stage+1}`);setText('hud-power',`LV.${game.player.level}`);
  setText('side-sector',STAGES[game.stage].name);setText('side-description',STAGES[game.stage].english);
  ...
  if(label.dataset.weapon!==weapon.name){label.innerHTML=`${weapon.name}<small>武器切替</small>`;label.dataset.weapon=weapon.name;label.style.color=weapon.color;}
